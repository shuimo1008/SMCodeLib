...
using ZCSharpLib.Zips.Encryption;
using ZCSharpLib.Zips.Core;
using ZCSharpLib.Zips.Checksum;
using ZCSharpLib.Zips.Zip.Compression.Streams;
using ZCSharpLib.Zips.Zip.Compression;
using System.Collections.Generic;
using ZCSharpLib.Zips.Additional;

namespace ZCSharpLib.Zips.Zip
{
	...
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		/// </exception>
		/// <exception cref="ZCSharpLib.SharpZip.Zip.ZipException">
					...
					if (testing && testData && this[entryIndex].IsFile) {
						...
						using (Stream entryStream = this.GetInputStream(this[entryIndex])) {
							...
							while ((bytesRead = entryStream.Read(buffer, 0, buffer.Length)) > 0) {
								crc.Update(new ZArraySegment<byte>(buffer, 0, bytesRead));
