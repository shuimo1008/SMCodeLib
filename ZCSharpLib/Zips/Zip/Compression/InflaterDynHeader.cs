...
using ZCSharpLib.Zips.Zip.Compression.Streams;
using ZCSharpLib.Zips.Additional;

namespace ZCSharpLib.Zips.Zip.Compression
{
	class InflaterDynHeader
	{
		...
		private IEnumerable<bool> CreateStateMachine()
		{
			...
			litLenTree = new InflaterHuffmanTree(new ZArraySegment<byte>(codeLengths, 0, litLenCodeCount));
			distTree = new InflaterHuffmanTree(new ZArraySegment<byte>(codeLengths, litLenCodeCount, distanceCodeCount));
